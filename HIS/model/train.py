...
from HIS.model.models import HIS
from HIS.model.minibatch import Minibatch
...
def prepare(train_data, train_params, arch_gcn):
    ...
    adj_full_norm = adj_norm(adj_full, model=train_params["model"])
    num_classes = class_arr.shape[1]
    minibatch = Minibatch(adj_full_norm, adj_train, role, train_params, feat_full)
    model = HIS(num_classes, arch_gcn, train_params, feat_full, class_arr)
    ...
    model_eval=HIS(num_classes, arch_gcn, train_params, feat_full, class_arr, cpu_eval=True)
...
if __name__ == '__main__':
    log_dir(args_global.train_config, args_global.data_prefix, timestamp)
    ...
    f1mic_test, f1mic_val = train(train_phases, model, minibatch, minibatch_eval, model_eval, train_params['eval_val_every'])
